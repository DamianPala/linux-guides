...
| `bash-completion/completions/` | Custom bash completions (manually added or generated by tools) |
...
# If logid.cfg has custom DPI — add hwdb rule so the desktop also uses it
sudo tee /etc/udev/hwdb.d/71-mouse-local.hwdb << 'EOF'
mouse:bluetooth:v046dpb025:name:MX Anywhere 3:*
mouse:usb:v046dpb025:name:MX Anywhere 3:*
 MOUSE_DPI=2000@1000
EOF
# Apply changes 
sudo systemd-hwdb update
# Restart the mouse (toggle power off/on)
...
|-----|-------------|--------------|
| PyCharm | `~/.config/JetBrains/PyCharmCE<ver>/` (classic — standard path) | Install fresh via `sudo snap install pycharm-community --classic`, launch, and use **File → Manage IDE Settings → Import Settings** pointing to the old config dir. This is cleaner than copying individual files |
